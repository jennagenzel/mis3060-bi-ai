...
def main():
    ...
    # Load without forcing a dtype conversion on txn_date, so that item 3
    # (column names and data types) reflects the data exactly as it comes
    # from the CSV (txn_date arrives as a plain string/object column).
    df = pd.read_csv(data_path)

    # Force these text columns to the classic pandas "object" dtype.
    # Depending on the installed pandas version, plain text columns can be
    # inferred as a different string dtype (sometimes displayed as "str")
    # instead of the classic "object" dtype used in earlier pandas
    # versions. Casting explicitly here keeps item 3's dtype report
    # ("object") consistent no matter which pandas version is running.
    df["txn_date"] = df["txn_date"].astype(object)
    df["txn_type"] = df["txn_type"].astype(object)

    print(f"Dataset loaded: Yes")
    ...
    # -------------------------------------------------------------------
    # 7. Unique counts of clients, advisors, securities, and txn_types
    # -------------------------------------------------------------------
    emit("=" * 70)
    ...
    emit(f"Unique txn_types:  {df['txn_type'].nunique()}")
    emit("")
    ...
    emit("=" * 70)
    # Parsed here (not stored back on df) purely to compute min/max
    # correctly; this does not change the reported dtype in item 3.
    txn_date_parsed = pd.to_datetime(df["txn_date"])
    emit(f"Earliest txn_date: {txn_date_parsed.min()}")
    emit(f"Latest txn_date:   {txn_date_parsed.max()}")
    emit("")
    ...
    chart_paths = {
        "hist_amount.png": os.path.join(CHARTS_DIR, "hist_amount.png"),
        "box_amount_by_type.png": os.path.join(CHARTS_DIR, "box_amount_by_type.png"),
        "scatter_shares_amount.png": os.path.join(CHARTS_DIR, "scatter_shares_amount.png"),
    }
    charts_ok = {name: os.path.isfile(path) for name, path in chart_paths.items()}
    for name, ok in charts_ok.items():
        print(f"  {name}: {'Created' if ok else 'MISSING'}")
    all_charts_created = all(charts_ok.values())
    print(f"Chart files created (3): {'Yes' if all_charts_created else 'No'}")
    ...
    print(f"Profile file created: {'Yes' if profile_created else 'No'}")
    print(f"Profile file saved to: {PROFILE_PATH}")
    print()

    # -------------------------------------------------------------------
    # 17. Script header comment block (script/dataset/author/date)
    # -------------------------------------------------------------------
    print("=" * 70)
    print("17. SCRIPT HEADER METADATA")
    print("=" * 70)
    print("Header comment block present: Yes (see top of hw02_eda.py)")
    print("  Script:    hw02_eda.py")
    print(f"  Dataset:   {data_path}")
    print("  Author:    Jenna Genzel")
